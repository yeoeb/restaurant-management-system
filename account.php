...
<div class="container-xxl flex-grow-1 container-p-y">
            ...
            <form method="get">
                <div class="row">
                    ...
                    <div class="col-md-4">
                        ...
                        <button type="reset" class="btn btn-secondary" onclick="location.href='<?php echo $base_url; ?>account.php'">取消</button>
                    </div>
                </div>
            </form>
        </div>
                    ...
                    <?php if ($users['status']): ?>
                        <?php foreach ($users['data'] as $index => $user): ?>
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-warning" onclick="location.href='<?php echo $base_url; ?>edit.php?id=<?php echo $user['id']; ?>'">修改</button>
    ...
    <a href="<?php echo $base_url; ?>add.php" class="btn btn-success mt-4">新增帳號</a>
</div>
...
function deleteUser(userId) {
    if (confirm("確定要刪除此用戶嗎？")) {
        // 發送刪除用戶的AJAX請求
        $.ajax({
            url: '<?php echo $base_url; ?>account.php',
            type: 'POST',
            data: {
...
function prevPage() {
    ...
    if (currentPage > 1) {
        location.href = '<?php echo $base_url; ?>account.php?page=' + (currentPage - 1);
    }
}
